...
struct Time
  ...
  def ssz_encode : Bytes
    to_unix_ms.ssz_encode
  end

  def ssz_encode(io : IO)
    to_unix_ms.ssz_encode(io)
  end

  def self.ssz_decode(io : IO, size : Int32)
    Time.unix_ms(Int64.ssz_decode(io, size))
  end
...
struct Time::Span
  ...
  def ssz_size : Int32
    sizeof(Int64) + sizeof(Int32)
  end

  def ssz_encode : Bytes
    to_i.ssz_encode + nanoseconds.ssz_encode
  end

  def ssz_encode(io : IO)
    to_i.ssz_encode(io)
    nanoseconds.ssz_encode(io)
  end

  def self.ssz_decode(io : IO, size : Int32)
    Time::Span.new(
      seconds: Int64.ssz_decode(io, sizeof(Int64)),
      nanoseconds: Int32.ssz_decode(io, sizeof(Int32))
    )
  end
